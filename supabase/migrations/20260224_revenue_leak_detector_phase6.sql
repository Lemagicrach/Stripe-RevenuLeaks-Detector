-- supabase/migrations/20260224_revenue_leak_detector_phase6.sql
-- Phase 6: SSE real-time alerts + recovery attribution to leak_id + team mode (shared checklist + assignments)

-- 1) Recovery attribution: link recovery events back to a specific leak
...
-- 2) Team mode: allow sharing a Stripe connection with multiple users
...
BEGIN
  ...
  -- Connection owner can manage memberships
  IF NOT EXISTS (
...
-- 3) Persisted assignments for Action Center (shared across team)
...
DO $$
BEGIN
  -- Allow owner and members to read
  IF NOT EXISTS (
  ...
  -- Allow owner and members to upsert (team workflow)
  IF NOT EXISTS (
